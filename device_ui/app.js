...
function linkedControls(){
  ...
  $('linked-state').textContent=!live?'Немає свіжого стану R1. Результат спільного запису невідомий.':active||p===6||p===7?((p===3&&c.linked_degraded?'Запис продовжується автономно. Зв’язок або стан одного з приладів втрачено; перевір після відновлення.':labels[p])+(c.linked_group&&c.linked_group!=='0'?' Сесія: '+c.linked_group:'')):p===undefined?'Спільний запис потребує оновлення прошивки R1.':toggle.checked?'Будь-яка кнопка Старт / Стоп керує CANBox і R1. Спільна сесія; CANBox починає першим.':'Увімкни «Спільно з R1», щоб будь-яка кнопка Старт / Стоп керувала обома записами.';
  if(linkedSelected()){
