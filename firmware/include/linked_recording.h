...
struct Coordinator {
    ...
    bool begin(uint64_t now){if(active())return false;phase=CanStarting;began=now;lastStop=0;degraded=false;return true;}
    ...
    uint8_t step(uint64_t now,const Observation& o){
        ...
        if(phase==CanStarting){
            ...
            if(o.r1Failed||now-began>30000000)fail();
            else if(o.r1Running)phase=Running;
        }else if(phase==Running){
            // Observation loss is not recorder failure. Keep surviving capture
            // running, including across a peer reboot; never blindly restart it.
            degraded=!o.fresh||!o.canOwned||!o.canRecording||!o.r1Running;
        }
        if(phase==Stopping||phase==Recovering){
            ...
            if(o.fresh&&o.canClosed&&now-began>8000000){phase=phase==Recovering||!o.canOwned?Failed:Closed;return None;}
