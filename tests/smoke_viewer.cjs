...
(async()=>{
 ...
 try{
  ...
  await page.locator('#start').fill('0.02');await page.locator('#end').fill('0.04');await page.locator('#apply').click();
  await page.waitForResponse(r=>r.url().includes('/api/window')&&r.ok());
