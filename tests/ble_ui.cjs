...
(async()=>{
 ...
 try{
  ...
  for(const id of ['ble-scan','ble-connect','ble-off','ble-peer','ble-pin'])assert.equal(await page.locator('#'+id).isDisabled(),true,'STOP-only '+id);
  ...
  console.log('BLE UI fixture PASS: discovery, PIN privacy, owner commands, recording guard, time uncertainty, counters, draft preservation, mobile layout.');
