...
| P4 — device control, acquisition and calibration | P4a: Fnirsi-style web UI, standalone OLED menus/buttons/encoder, core/task ownership and P3 settings controls; P4b: production INA driver, timing, queue, raw/valid samples | Fresh cadence measured under OLED/input/network load; +/- current and voltage points compared; gap handling and applied settings verified |
...
| P6 — UI integration and usability | Refine local/web screens, network setup, versioned live API and storage browsing after the P4a/P5 controls work | READY/RUN/STOPPING/ERROR visible locally and remotely; controls obey state gates; operation with Wi-Fi off and browser disconnect verified; offline logs remain usable |
